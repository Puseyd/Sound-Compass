...
/*
DOCS:
https://forum.arduino.cc/t/how-do-i-get-stable-readings-from-the-piezo-sensor/955665
*/
...
void loop() {
  ...
  */
    /*while(sensorReading1 >= threshold){
    ...
    /*
    if (sensorReading1 >= threshold && (count1 % 2 == 0)){
      for(int i = 0; i <= 127; i++){
        analogWrite(redLedPin, i);
        delay(20);
      }
      ...
      count1++;
    } else if (sensorReading1 >= threshold && (count1 % 2 == 1)){
      for (int i = 127; i>=0; i--){
        analogWrite(redLedPin, i);
        delay(10);
  ...
  if ( (sensorReading1 >= threshold) && (ledState == LOW))   {
    for(int i = 0; i <= 127; i++){
        analogWrite(redLedPin, i);
        delay(20);
      }
   ...
   else if((sensorReading1 >= threshold) && (ledState == HIGH) ) {
    for (int i = 127; i>=0; i--){
        analogWrite(redLedPin, i);
        delay(10);
  }
